...
当前 `MainWindow` 使用标签页分别创建 `MinesweeperDocument` 与 `SpiderSolitaireDocument`，并承载各自的 SDK
边界包装 View。两个包装 View 都通过单向绑定把 Document 拥有的 ViewModel 交给真正的游戏 View。该结构用于快速
检查扫雷和蜘蛛纸牌的布局与交互；规则、计时和状态转换仍全部来自 Plugin 项目，Standalone 不维护第二份实现，
窗口关闭时也会释放两个 Document 拥有的计时与动画资源。
