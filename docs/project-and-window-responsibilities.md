...
当前 `MainWindow` 使用八个标签页分别创建扫雷、蜘蛛纸牌、黑白棋、五子棋、中国象棋、2048、数独和推箱子 Document，并承载各自的
SDK 边界包装 View。八个包装 View 都通过单向绑定把 Document 拥有的 ViewModel 交给真正的游戏 View。该结构用于快速
检查全部游戏的布局与交互；规则、计时和状态转换仍全部来自 Plugin 项目，Standalone 不维护第二份实现。窗口关闭时只释放
确实拥有计时、动画、后台电脑或题目生成资源的 Document；数独会取消生成并停止计时，2048 与推箱子的动画计时器只存在于视觉树中，
两个 Document 都没有外部资源，不增加空洞的释放行为。
