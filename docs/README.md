...
- [富翁小镇 3D 专用方案](rich-town.md)：Stride 集成、SOLID 职责、规则与完整测试矩阵；**G2 确定性规则已实现，G1 集成仍阻塞**。
...
统一 `verify` / `seal`、Windows CI 或发布门禁，不使用 AIFLOW。G2 只执行无窗口的本地规则检查，不部署、不启动 Host/GPU。
