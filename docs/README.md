...
本解决方案用于开发 `myavalonia.plugin.classic.game` Managed Plugin，当前由独立扫雷功能域提供一个普通
Document。它把真实插件、独立 Avalonia 开发窗口和自动化测试放在同一个解决方案中，使界面与业务代码既能
...
- 扫雷开发阶段只要求 Debug 警告即错误和全量单元测试通过，不运行 Windows CI、Release 打包或发布门禁。
