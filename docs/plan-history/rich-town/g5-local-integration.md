...
> 后续更新：用户单独授权部署修复与发布后，DependencyModel、资源映射和真实 Host SDL 加载问题已解决；1.2.0 包已在原桌面 Host 副本显示棋盘并推进回合。见[部署修复记录](g5-deployment-repair.md)。下文保留修复前证据，叠层、只读初始化与句柄增长仍未签署。
